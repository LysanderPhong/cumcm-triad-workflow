...
适合以单人参赛、AI 协作、争取省级以上成绩为目标的数模训练或 CUMCM 参赛工作；成绩是使用者的目标，不是本 Skill 的能力证明。它与阶段专业技能（数据审计、方法筛选、代码生成、稳健性检验、论文写作等）互补：那些技能组织各阶段产物，本 Skill 约束决策权、执行范围和独立审核；本 Skill 自包含，不依赖任何特定技能包。当前发布候选为 Experimental / Release Candidate：部分机制有 Benchmark 02 历史单轮证据，当前打包版本的冷启动行为仍待验证。参赛资格与当年规则另行核实。
