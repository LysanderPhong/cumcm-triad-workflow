...
def flags(candidate: dict, decision_count: int | None) -> list[dict]:
    result: list[dict] = []
...
def table_rows(candidate: dict, references: list[tuple[str, dict]]) -> str:
    keys = ("chinese_chars", "paragraphs", "sentences", "headings", "figure_mentions", "table_mentions", "citation_markers", "equation_markers", "avg_sentence_chars")
    labels = {"chinese_chars": "中文字符", "paragraphs": "段落", "sentences": "句子", "headings": "标题", "figure_mentions": "图引用", "table_mentions": "表引用", "citation_markers": "引用标记", "equation_markers": "公式标记", "avg_sentence_chars": "平均句长"}
...
def main() -> int:
    ...
    for path in args.reference:
        ...
        if ref_text.strip():
            references.append((path.name, metrics(ref_text)))
    count = decision_count(args.decision_log)
    findings = flags(candidate, count)
